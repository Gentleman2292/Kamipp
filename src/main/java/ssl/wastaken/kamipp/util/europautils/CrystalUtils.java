...
public class CrystalUtils implements IMinecraft
{
    public static EntityPlayer getTarget(final float range) {
        ...
        for (final EntityPlayer player : new ArrayList<EntityPlayer>(CrystalUtils.mc.world.playerEntities)) {
            ...
            if (Kami.friendManager.isFriend(player.getName())) {
                continue;
            }
            if (player.isDead) {
                continue;
   ...
   public static void drawText(final BlockPos pos, final String text) {
        ...
        GlStateManager.translate(-(Kami.fontManager.getStringWidth(text) / Double.longBitsToDouble(Double.doubleToLongBits(0.2579471275262499) ^ 0x7FD08234AB34A1F3L)), Double.longBitsToDouble(Double.doubleToLongBits(8.964569445934237E307) ^ 0x7FDFEA38A13E706BL), Double.longBitsToDouble(Double.doubleToLongBits(1.3014105849625204E308) ^ 0x7FE72A76F089AAE7L));
        Kami.fontManager.drawString(text, Float.intBitsToFloat(Float.floatToIntBits(2.2813044E36f) ^ 0x7BDBAE7F), Float.intBitsToFloat(Float.floatToIntBits(1.2346908E38f) ^ 0x7EB9C68F), Color.WHITE);
