...
import ssl.wastaken.kamipp.features.modules.player.Speedmine;
...
public class RenderUtil
    ...
    void boxESP(BlockPos blockPos, Color color, float f, boolean bl, boolean bl2, int n, boolean bl3) {
        ...
        if (camera.isBoundingBoxInFrustum(new AxisAlignedBB(axisAlignedBB.minX + RenderUtil.mc.getRenderManager ().viewerPosX, axisAlignedBB.minY + RenderUtil.mc.getRenderManager().viewerPosY , axisAlignedBB.minZ + RenderUtil.mc.getRenderManager().viewerPosZ, axisAlignedBB.maxX + RenderUtil.mc.getRenderManager().viewerPosX, axisAlignedBB.maxY + RenderUtil.mc.getRenderManager().viewerPosY, axisAlignedBB.maxZ + RenderUtil.mc.getRenderManager().viewerPosZ))) {
            ...
            float f2 = (float) Speedmine.getInstance().timer.getPassedTimeMs() / 1000.0f / (Speedmine.getInstance().breakTime * Kami.serverManager.getTpsFactor());
            f2 = Math.min (f2 , 1.0f);
            if (bl3) {
    ...
    public static void drawWaypointImage(BlockPos pos, GLUProjection.Projection projection, Color color, String name, boolean rectangle, Color rectangleColor) {
        ...
        float textWidth = Kami.textManager.getStringWidth(text);
        Kami.textManager.drawString(text, -(textWidth / 2.0f), -((float) Kami.textManager.getFontHeight() / 2.0f) + (float) Kami.textManager.getFontHeight() / 2.0f, color.getRGB(), false);
        GlStateManager.translate(-projection.getX(), -projection.getY(), 0.0);
        GlStateManager.popMatrix();
    }
    ...
    public static void drawText(final AxisAlignedBB pos, final String text) {
        ...
        GlStateManager.disableDepth();
        GlStateManager.translate(-(Kami.textManager.getStringWidth(text) / 2.0), 0.0, 0.0);
        Kami.textManager.drawStringWithShadow(text, 0.0f, 0.0f, -5592406);
        GlStateManager.popMatrix();
    }
    ...
    public static void drawProjectedText(GLUProjection.Projection projection, float addX, float addY, String text, Color color, boolean shadow) {
        if (projection.getType() == GLUProjection.Projection.Type.INSIDE) {
            GlStateManager.pushMatrix();
            GlStateManager.translate(projection.getX(), projection.getY(), 0.0);
            Kami.textManager.drawString(text, -((float) Kami.textManager.getStringWidth(text) / 2.0f) + addX, addY, color.getRGB(), shadow);
            GlStateManager.translate(-projection.getX(), -projection.getY(), 0.0);
            GlStateManager.popMatrix();
        }
    }
