...
import ssl.wastaken.kamipp.features.modules.combat.Killaura;
import ssl.wastaken.kamipp.features.modules.player.FakePlayer;
import ssl.wastaken.kamipp.features.modules.player.Freecam;
import ssl.wastaken.kamipp.mixin.mixins.accessors.IEntityLivingBase;
...
public class EntityUtil implements Util {
    ...
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals(mc.player) || (entity instanceof EntityPlayer && Kami.friendManager.isFriend(entity.getName())) || mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(final Entity entity, final int red, final int green, final int blue, final int alpha, final boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer) {
            if (colorFriends && Kami.friendManager.isFriend((EntityPlayer) entity)) {
                color = new Color(0.33333334f, 1.0f, 1.0f, alpha / 255.0f);
            }
            final Killaura killaura = Kami.moduleManager.getModuleByClass(Killaura.class);
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : mc.world.playerEntities) {
            ...
            output.put(healthSB.toString() + " " + (Kami.friendManager.isFriend(player) ? "\u00a7b" : "\u00a7r") + player.getName() + " " + distanceSB.toString() + " " + "\u00a7f" + Kami.totemPopManager.getTotemPopString(player) + Kami.potionManager.getTextRadarPotion(player), (int) mc.player.getDistance(player));
