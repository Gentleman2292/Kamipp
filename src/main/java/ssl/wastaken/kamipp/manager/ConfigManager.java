...
import ssl.wastaken.kamipp.features.setting.EnumConverter;
...
public class ConfigManager
    ...
    public String config = "Kami/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        switch (setting.getType()) {
            ...
            default: {
                Kami.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
            }
        }
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    Kami.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
            ...
            AntiDDoS noDDoS = (AntiDDoS) feature;
    ...
    public void loadConfig(String name) {
        ...
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("Kami").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        this.config = files.contains(new File("Kami/" + name + "/")) ? "Kami/" + name + "/" : "Kami/config/";
    ...
    public void saveConfig(String name) {
        ...
        this.config = "Kami/" + name + "/";
        File path = new File(this.config);
        if (!path.exists()) {
            path.mkdir();
        }
        Kami.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("Kami/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("SexMaster", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(Kami.moduleManager.modules);
        this.features.add(Kami.friendManager);
        String name = this.loadCurrentConfig();
        this.loadConfig(name);
        Kami.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            Kami.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
