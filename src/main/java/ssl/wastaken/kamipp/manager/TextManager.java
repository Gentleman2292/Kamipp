...
import ssl.wastaken.kamipp.features.modules.client.FontMod;
import ssl.wastaken.kamipp.util.CustomFont;
...
public class TextManager
    ...
    private final CustomFont smallString = new CustomFont(new Font("tahoma", 1, 12), true, false);
    ...
    public void init(boolean startup) {
        FontMod cFont = Kami.moduleManager.getModuleByClass(FontMod.class);
        try {
    ...
    public float drawString(String text, float x, float y, int color, boolean shadow) {
        if (Kami.moduleManager.isModuleEnabled(FontMod.class)) {
            if (shadow) {
    ...
    public static void drawStringWithShadow(String text, float x, float y, int colour, boolean shadow) {
        if (!shadow) {
