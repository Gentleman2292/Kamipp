...
import ssl.wastaken.kamipp.event.events.BlockEvent;
...
class Speedmine
    ...
    void onTick ( ) {
        if ( this.currentPos != null ) {
            ...
            if ( Speedmine.mc.player != null && this.silentSwitch.getValue ( ) && this.timer.passedMs ( (int) ( 2000.0f * Kami.serverManager.getTpsFactor ( ) ) ) && this.getPickSlot ( ) != - 1 ) {
                Speedmine.mc.player.connection.sendPacket ( new CPacketHeldItemChange ( this.getPickSlot ( ) ) );
            }
            if ( Speedmine.mc.player != null && this.silentSwitch.getValue ( ) && this.timer.passedMs ( (int) ( 2200.0f * Kami.serverManager.getTpsFactor ( ) ) ) ) {
