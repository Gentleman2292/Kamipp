...
import ssl.wastaken.kamipp.features.modules.player.XCarry;
...
public class AutoArmor
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState() && !(AutoArmor.mc.currentScreen instanceof KamippGui) && this.elytraBind.getValue().getKey() == Keyboard.getEventKey()) {
    ...
    public void onTick() {
        ...
        if (this.timer.passedMs((int) ((float) this.delay.getValue().intValue() * (this.tps.getValue() != false ? Kami.serverManager.getTpsFactor() : 1.0f)))) {
            if (!this.taskList.isEmpty()) {
