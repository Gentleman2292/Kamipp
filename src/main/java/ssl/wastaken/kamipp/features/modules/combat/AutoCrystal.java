...
import ssl.wastaken.kamipp.event.events.UpdateWalkingPlayerEvent;
...
public class AutoCrystal
    ...
    public final ssl.wastaken.kamipp.util.Timer threadTimer = new ssl.wastaken.kamipp.util.Timer();
    ...
    private final ssl.wastaken.kamipp.util.Timer switchTimer = new ssl.wastaken.kamipp.util.Timer();
    private final ssl.wastaken.kamipp.util.Timer manualTimer = new ssl.wastaken.kamipp.util.Timer();
    private final ssl.wastaken.kamipp.util.Timer breakTimer = new ssl.wastaken.kamipp.util.Timer();
    private final ssl.wastaken.kamipp.util.Timer placeTimer = new ssl.wastaken.kamipp.util.Timer();
    private final ssl.wastaken.kamipp.util.Timer syncTimer = new ssl.wastaken.kamipp.util.Timer();
    private final ssl.wastaken.kamipp.util.Timer predictTimer = new ssl.wastaken.kamipp.util.Timer();
    private final ssl.wastaken.kamipp.util.Timer renderTimer = new ssl.wastaken.kamipp.util.Timer();
    ...
    private final ssl.wastaken.kamipp.util.Timer syncroTimer = new ssl.wastaken.kamipp.util.Timer();
    private final Map<EntityPlayer, ssl.wastaken.kamipp.util.Timer> totemPops = new ConcurrentHashMap<EntityPlayer, ssl.wastaken.kamipp.util.Timer>();
    ...
    public Setting<Boolean> fakeSwing = this.register(new Setting<Object>("FakeSwing", Boolean.valueOf(false), v -> this.setting.getValue() == Settings.DEV));
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (!this.justRender.getValue() && this.switchTimer.passedMs(this.switchCooldown.getValue()) && this.explode.getValue() && this.instant.getValue() && event.getPacket() instanceof SPacketSpawnObject && (this.syncedCrystalPos == null || !this.syncedFeetPlace.getValue() || this.damageSync.getValue() == DamageSync.NONE)) {
            ...
            if (packet5.getOpCode() == 35 && packet5.getEntity((World)AutoCrystal.mc.world) instanceof EntityPlayer) {
                this.totemPops.put((EntityPlayer)packet5.getEntity((World)AutoCrystal.mc.world), new ssl.wastaken.kamipp.util.Timer().reset());
            }
            ...
            if (this.sound.getValue().booleanValue() || this.threadMode.getValue() == ThreadMode.SOUND) {
                AutoCrystal.removeEntities(packet, this.soundRange.getValue().floatValue());
            }
    ...
    public static void removeEntities(final SPacketSoundEffect packet, final float range) {
        ...
        final List<Entity> copy = new CopyOnWriteArrayList<Entity>(AutoCrystal.mc.world.loadedEntityList);
    ...
    private boolean isRightThread() {
        return mc.isCallingFromMinecraftThread() || !Kami.eventManager.ticksOngoing() && !this.threadOngoing.get();
    }
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState() && !(AutoCrystal.mc.currentScreen instanceof KamippGui) && this.switchBind.getValue().getKey() == Keyboard.getEventKey()) {
            if (this.switchBack.getValue().booleanValue() && this.offhandSwitch.getValue().booleanValue() && this.offHand) {
                Offhand module = Kami.moduleManager.getModuleByClass(Offhand.class);
                if (module.isOff()) {
        ...
        for (Entity entity : AutoCrystal.mc.world.loadedEntityList) {
            ...
            if ((double)selfDamage + 0.5 < (double)EntityUtil.getHealth((Entity)AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue().floatValue()) {
                ...
                for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                    ...
                    if (this.antiFriendPop.getValue() != AntiFriendPop.BREAK && this.antiFriendPop.getValue() != AntiFriendPop.ALL || !Kami.friendManager.isFriend(player.getName()) || !((double)(damage = DamageUtil.calculateDamage(entity, (Entity)player)) > (double)EntityUtil.getHealth((Entity)player) + 0.5)) continue;
                    ...
                    if (syncflag || this.currentDamage - (double)damageOffset > this.lastDamage || this.syncTimer.passedMs(this.damageSyncTime.getValue().intValue()) || this.damageSync.getValue() == DamageSync.NONE) {
                        ...
                        if (this.switchItem()) {
                            ...
                            if (this.foundDoublePop) {
                                this.totemPops.put(target, new ssl.wastaken.kamipp.util.Timer().reset());
                            }
        ...
        block0: for (BlockPos pos : BlockUtil.possiblePlacePositions(this.placeRange.getValue().floatValue(), this.antiSurround.getValue(), this.oneDot15.getValue())) {
            ...
            if (targettedPlayer != null) {
                ...
                if (this.calcEvenIfNoDamage.getValue().booleanValue() && (this.antiFriendPop.getValue() == AntiFriendPop.ALL || this.antiFriendPop.getValue() == AntiFriendPop.PLACE)) {
                    ...
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        float friendDamage;
                        if (friend == null || AutoCrystal.mc.player.equals((Object)friend) || friend.getDistanceSq(pos) > MathUtil.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue()) || !Kami.friendManager.isFriend(friend) || !((double)(friendDamage = DamageUtil.calculateDamage(pos, (Entity)friend)) > (double)EntityUtil.getHealth((Entity)friend) + 0.5)) continue;
                        friendPop = true;
                        break;
                    }
        ...
        switch (this.rotate.getValue()) {
            ...
            case ALL: {
                ...
                if (this.eventMode.getValue() == 2 && this.threadMode.getValue() == ThreadMode.NONE) {
                    Kami.rotationManager.setPlayerRotations(angle[0], angle[1]);
                    break;
                }
                this.yaw = angle[0];
                this.pitch = angle[1];
        ...
        public void run() {
            if (this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                while (this.autoCrystal.isOn() && this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                    while (Kami.eventManager.ticksOngoing()) {
                    }
                    ...
                    Kami.safetyManager.doSafetyCheck();
