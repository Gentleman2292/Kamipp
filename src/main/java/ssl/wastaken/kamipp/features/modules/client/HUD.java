...
import ssl.wastaken.kamipp.features.modules.misc.ToolTips;
import ssl.wastaken.kamipp.manager.TextManager;
...
public class HUD
    ...
    private final Setting<Boolean> server = register(new Setting("Server", Boolean.valueOf(false), "Shows the server ip"));
    ...
    public void onUpdate() {
        for (final Module module : Kami.moduleManager.sortedModules) {
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        switch (this.watermark.getValue()) {
            case SEXMASTER: {
                this.renderer.drawString("Kami++" + (this.modeVer.getValue() ? " v6.9" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
                break;
            }
            case CHACHOOX: {
                this.renderer.drawString("Kami++" + (this.modeVer.getValue() ? " v6.9" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2) : this.color, true);
        ...
        if (this.arrayList.getValue()) {
            if (this.renderingUp.getValue()) {
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? Kami.moduleManager.alphabeticallySortedModules.size() : Kami.moduleManager.sortedModules.size()); ++k) {
                    final Module module = this.alphabeticalSorting.getValue() ? Kami.moduleManager.alphabeticallySortedModules.get(k) : Kami.moduleManager.sortedModules.get(k);
                    final String text = module.getDisplayName() + "\u00a77" + ((module.getDisplayInfo() != null) ? (" [\u00a7f" + module.getDisplayInfo() + "\u00a77" + "]") : "");
                    this.renderer.drawString(text, width - 2 - this.renderer.getStringWidth(text) + ((this.animationHorizontalTime.getValue() == 1) ? 0.0f : module.arrayListOffset), (float) (2 + j * 10), (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(MathUtil.clamp(2 + j * 10, 0, height)) : this.color, true);
                    ++j;
                }
            } else {
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? Kami.moduleManager.alphabeticallySortedModules.size() : Kami.moduleManager.sortedModules.size()); ++k) {
                    final Module module = this.alphabeticalSorting.getValue() ? Kami.moduleManager.alphabeticallySortedModules.get(Kami.moduleManager.alphabeticallySortedModules.size() - 1 - k) : Kami.moduleManager.sortedModules.get(k);
                    final String text = module.getDisplayName() + "\u00a77" + ((module.getDisplayInfo() != null) ? (" [\u00a7f" + module.getDisplayInfo() + "\u00a77" + "]") : "");
                    ...
                    final int n = height;
                    j += 10;
                    renderer.drawString(text5, x, (float) (n - j), (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(MathUtil.clamp(height - j, 0, height)) : this.color, true);
                }
            }
        }
        ...
        if (this.renderingUp.getValue()) {
            if (this.serverBrand.getValue()) {
                final String text2 = grayString + "Server brand " + "\u00a7f" + Kami.serverManager.getServerBrand();
            ...
            if (this.potions.getValue()) {
                for (final PotionEffect effect : Kami.potionManager.getOwnPotions()) {
                    final String text3 = this.altPotionsColors.getValue() ? Kami.potionManager.getPotionString(effect) : Kami.potionManager.getColoredPotionString(effect);
            ...
            if (this.server.getValue().booleanValue()) {
                String text2 = grayString + "Server " + "\u00a7f" + (Util.mc.isSingleplayer() ? "SinglePlayer" : Objects.requireNonNull ( Util.mc.getCurrentServerData ( ) ).serverIP);
                final TextManager renderer4 = this.renderer;
                final String text16 = text2;
                final float x4 = (float) (width - (this.renderer.getStringWidth(text2) + 2));
                final int n4 = height - 2;
                k += 10;
                renderer4.drawString(text16, x4, (float) (n4 - k), (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(height - k) : this.color, true);
            }
            if (this.speed.getValue()) {
                final String text2 = grayString + "Speed " + "\u00a7f" + Kami.speedManager.getSpeedKpH() + " km/h";
                final TextManager renderer4 = this.renderer;
            ...
            if (this.tps.getValue()) {
                final String text2 = grayString + "TPS " + "\u00a7f" + Kami.serverManager.getTPS();
            ...
            final String text = grayString + "Ping " + "\u00a7f" + (Kami.serverManager.getPing()) + (this.MS.getValue() ? "ms" : "");
            ...
            final String fpsText = grayString + "FPS " + "\u00a7f" + Minecraft.debugFPS;
            final String text = grayString + "Ping " + "\u00a7f" + Kami.serverManager.getPing();
            if (this.renderer.getStringWidth(text) > this.renderer.getStringWidth(fpsText)) {
                if (this.ping.getValue()) {
                    this.renderer.drawString(text, (float) (width - (this.renderer.getStringWidth(text) + 2)), (float) (2 + k++ * 10), (this.rolling.getValue() && this.rainbow.getValue()) ? this.colorMap.get(2 + k * 10) : this.color, true);
                }
        ...
        final String text4 = (this.direction.getValue() ? (Kami.rotationManager.getDirection4D(false) + " ") : "") + (this.coords.getValue() ? coordinates : "") + "";
    ...
    public void renderGreeter() {
        ...
        switch (this.greeter.getValue()) {
            ...
            case LONG: {
                text = text + "Looking nice today " + HUD.mc.player.getDisplayNameString() + " 8^)";
                break;
            }
            default: {
                text = text + "Welcome to Kami++ " + HUD.mc.player.getDisplayNameString();
                break;
            }
        }
    ...
    public void renderLag() {
        final int width = this.renderer.scaledWidth;
        if (Kami.serverManager.isServerNotResponding()) {
            final String text = ((this.lag.getValue() == LagNotify.GRAY) ? "\u00a77" : "\u00a7c") + "Server not responding: " + MathUtil.round(Kami.serverManager.serverRespondingTime() / 3000.0f, 1) + "s.";
