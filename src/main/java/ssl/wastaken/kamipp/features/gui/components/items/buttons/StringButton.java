...
public class StringButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.isListening) {
            Kami.textManager.drawStringWithShadow(this.currentString.getString() + Kami.textManager.getIdleSign(), this.x + 2.3f, this.y - 1.7f - (float) KamippGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        } else {
            Kami.textManager.drawStringWithShadow((this.setting.shouldRenderName() ? this.setting.getName() + " \u00a77" : "") + this.setting.getValue(), this.x + 2.3f, this.y - 1.7f - (float) KamippGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
