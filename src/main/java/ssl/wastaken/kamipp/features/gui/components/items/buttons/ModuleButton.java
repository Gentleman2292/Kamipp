...
public class ModuleButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (!this.items.isEmpty()) {
            ClickGui gui = Kami.moduleManager.getModuleByClass(ClickGui.class);
            Kami.textManager.drawStringWithShadow(gui.openCloseChange.getValue().booleanValue() ? (this.subOpen ? gui.close.getValue() : gui.open.getValue()) : gui.moduleButton.getValue(), this.x - 1.5f + (float)this.width - 7.4f, this.y - 2.0f - (float) KamippGui.getClickGui().getTextOffset(), -1);
        }
        if (this.subOpen) {
            float height = 1.0f;
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (!this.items.isEmpty()) {
            if (mouseButton == 1 && this.isHovering(mouseX, mouseY)) {
                ...
                Util.mc.getSoundHandler().playSound((ISound)PositionedSoundRecord.getMasterRecord((SoundEvent)SoundEvents.BLOCK_ANVIL_FALL, (float)1.0f));
            }
            if (this.subOpen) {
                for (Item item : this.items) {
